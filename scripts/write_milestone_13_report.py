report_content = """# Milestone 13: Human vs. LLM Judge Validation Report (`@AmazonHelp`)
...
1. **Strong Global Alignment:** Across all 100 paired ratings (20 cases $\times$ 5 dimensions), the LLM judge achieved a **Pearson correlation of $r = 0.827$ ($p < 10^{-25}$)**, a **Spearman rank correlation of $\rho = 0.840$ ($p < 10^{-26}$)**, and a **Quadratic Weighted Kappa (QWK) of $0.791$**, signifying substantial inter-rater reliability.
...
3. **Exceptional Calibration on Correctness & Helpfulness:** Correctness achieved $r = 0.925$ ($\rho = 0.904$, 100% adjacent agreement, $\text{QWK} = 0.820$); Helpfulness achieved $r = 0.889$ ($\rho = 0.890$, 100% adjacent agreement, $\text{QWK} = 0.877$).
...
| Dimension | Human Mean | LLM Mean | Mean Bias $\Delta$ | Pearson $r$ | Spearman $\rho$ | MAE | RMSE | Exact Agree | Adjacent Agree ($\pm 1$) | Quadratic Weighted Kappa (QWK) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
### 1. Correctness ($r = 0.925$, $\rho = 0.904$, $\text{QWK} = 0.820$)
...
### 2. Helpfulness ($r = 0.889$, $\rho = 0.890$, $\text{QWK} = 0.877$)
...
with open("reports/milestone_13_human_vs_llm_validation.md", "w", encoding="utf-8") as f:
    f.write(report_content)

print("[OK] Successfully generated reports/milestone_13_human_vs_llm_validation.md")
